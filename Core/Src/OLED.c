...
void OLED_GPIO_Init(void)
{
	...
	//在初始化前，加入适量延时，待OLED供电稳定#1#
	for (i = 0; i < 1000; i ++)
	{
		for (j = 0; j < 1000; j ++);
	}

	//将D0、D1、RES、DC和CS引脚初始化为推挽输出模式#1#
	...
	//OLED_W_CS(1);
	OLED_W_RES(0);
	HAL_Delay(10);          // 保持低电平至少 10ms
	OLED_W_RES(1);
	HAL_Delay(10);          // 等待 OLED 稳定
}
...
void OLED_SPI_SendByte(uint8_t Byte)
{
	...
	for (i = 0; i < 8; i++)
	{
		/*使用掩码的方式取出Byte的指定一位数据并写入到D1线#1#
		/*两个!的作用是，让所有非零的值变为1#1#
		OLED_W_D1(!!(Byte & (0x80 >> i)));
		OLED_W_D0(1);	//拉高D0，从机在D0上升沿读取SDA
		OLED_W_D0(0);	//拉低D0，主机开始发送下一位数据
	}*/
	HAL_SPI_Transmit_DMA(&hspi3, &Byte, 1);	//使用HAL库函数发送一个字节数据
}
